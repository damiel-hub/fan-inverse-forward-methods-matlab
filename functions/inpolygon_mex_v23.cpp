 ...
 *
 * (FIX 25 - MEMORY) This version avoids allocating M*N intermediate
 * arrays. It now allocates intermediate 'diffCount' and 'isInside'
 * arrays sized only to the polygon's bounding box (M_sub * N_sub),
 * drastically reducing memory usage for small polygons on large grids.
 *
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    // --- 5. Find Polygon BBox and Scan Direction ---
    ...
    const size_t M_sub = (r_end_bbox > r_start_bbox) ? (r_end_bbox - r_start_bbox) : 0;
    const size_t N_sub = (c_end_bbox > c_start_bbox) ? (c_end_bbox - c_start_bbox) : 0;
    
    // --- !! NEW !!: Check for empty sub-grid ---
    if (M_sub == 0 || N_sub == 0) {
        // Polygon is completely outside the grid, outputs are already false.
        return; 
    }
    
    const bool useYScan = (M_sub <= N_sub); 

    // --- 6. Allocate Intermediates (MODIFIED) ---
    // --- This is the main memory fix ---
    std::vector<int> diffCount_sub(M_sub * N_sub, 0); 
    std::vector<bool> isInsideY_sub(M_sub, false); 
    std::vector<bool> isInsideX_sub(N_sub, false); 
    ...
    for (size_t p = 0; p < nan_indices.size() - 1; ++p) {
        ...
        // --- 7.B: Part B (Scan-Line) C++ Implementation (MODIFIED) ---
        if (useYScan) {
            // --- Y-SCAN ---
            for (size_t e = 0; e < V - 1; ++e) {
                Point p1 = poly_pts[e];
                ...
                // --- !! NEW !!: Clip edge scan-lines to polygon BBox ---
                r_start = std::max(r_start, r_start_bbox);
                r_end = std::min(r_end, r_end_bbox);
                ...
                for (size_t r = r_start; r < r_end; ++r) {
                    ...
                    // --- !! NEW !!: Get sub-grid row index ---
                    size_t r_sub = r - r_start_bbox; 
                    
                    if (x_intersect < value_x[0] - EPSILON) {
                        isInsideY_sub[r_sub] = !isInsideY_sub[r_sub]; // Use sub-array
                    } 
                    else if (x_intersect <= value_x[N-1] + EPSILON) {
                        size_t c = findNextGridIndex(value_x, N, x_intersect);
                        
                        // --- !! NEW !!: Check against BBox and use sub-grid indices ---
                        if (c >= c_start_bbox && c < c_end_bbox) {
                            size_t c_sub = c - c_start_bbox;
                            size_t idx_sub = c_sub * M_sub + r_sub;
                            diffCount_sub[idx_sub]++; // Use sub-array
                        }
                    }
            ...
            for (size_t e = 0; e < V - 1; ++e) {
                ...
                // --- !! NEW !!: Clip edge scan-lines to polygon BBox ---
                c_start = std::max(c_start, c_start_bbox);
                c_end = std::min(c_end, c_end_bbox);
    ...
    // --- 8. Finalize Outputs (MODIFIED) ---
    if (useYScan) {
        // Y-Scan: cumsum along dimension 2 (X)
        #pragma omp parallel for
        for (size_t r_sub = 0; r_sub < M_sub; ++r_sub) { // Loop over sub-grid
            size_t r = r_sub + r_start_bbox; // Map to full grid
            int crossingCount = 0;
            bool parity = isInsideY_sub[r_sub]; // Use sub-array
            
            for (size_t c_sub = 0; c_sub < N_sub; ++c_sub) { // Loop over sub-grid
                size_t c = c_sub + c_start_bbox; // Map to full grid
                
                size_t idx_sub = c_sub * M_sub + r_sub; // Sub-grid index
                size_t idx_full = c * M + r;          // Full-grid index
                
                crossingCount += diffCount_sub[idx_sub]; // Read from sub-array
                bool totalInclusion = (crossingCount % 2) != 0;
                totalInclusion = totalInclusion ^ parity;

                if (!onedgeMeshPtr[idx_full]) { 
                    inMeshPtr[idx_full] = totalInclusion; // Write to full-array
                }
            }
        }
    } else {
        ...
        for (size_t c_sub = 0; c_sub < N_sub; ++c_sub) { // Loop over sub-grid
            size_t c = c_sub + c_start_bbox; // Map to full grid
            int crossingCount = 0;
            bool parity = isInsideX_sub[c_sub]; // Use sub-array
