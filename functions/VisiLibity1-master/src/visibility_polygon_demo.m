...
while 0 < 1
    ...
    [vis_poly, growing_verts] = visibility_polygon_v2( [observer_x observer_y] , environment , epsilon , snap_distance );
    patch( vis_poly(:,1) , vis_poly(:,2) , 0.1*ones( size(vis_poly,1) , 1 ) , ...
           'r' , 'linewidth' , 1.5 );
    plot3( vis_poly(:,1) , vis_poly(:,2) , 0.1*ones( size(vis_poly,1) , 1 ) , ...
           'b*' , 'Markersize' , 5 );

    plot3( growing_verts(:,1) , growing_verts(:,2) , 0.1*ones( size(growing_verts,1) , 1 ) , ...
           'g*' , 'Markersize' , 5 );
