/* ========================================================================
 * inpolygon_mex_v23_serial.cpp
 *
 * This version is a "parfor-safe" serial implementation.
 * All OpenMP directives have been removed. This function
 * is intended to be called from *within* a MATLAB parfor loop,
 * allowing MATLAB to handle the parallelization.
 *
 * It retains the memory optimization from FIX 25, allocating
 * intermediate arrays only for the polygon's bounding box.
 *
 * COMPILATION (Windows):
 * mex -v CXXFLAGS="$CXXFLAGS /std:c++17 /O2" inpolygon_mex_v23_serial.cpp
 *
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    // --- 6. Allocate Intermediates (Memory Fix) ---
    ...
    for (size_t p = 0; p < nan_indices.size() - 1; ++p) {
        ...
        // --- #pragma omp parallel for REMOVED ---
        ...
        // --- 7.B: Part B (Scan-Line) C++ Implementation (Memory Fix) ---
        if (useYScan) {
            // --- Y-SCAN ---
            for (size_t e = 0; e < V - 1; ++e) {
                Point p1 = poly_pts[e];
                ...
                if (std::abs(p1.y - p2.y) < EPSILON) continue;
                ...
                for (size_t r = r_start; r < r_end; ++r) {
                    ...
                    if (x_intersect < value_x[0] - EPSILON) {
                        isInsideY_sub[r_sub] = !isInsideY_sub[r_sub];
                    } 
            ...
            for (size_t e = 0; e < V - 1; ++e) {
                ...
                if (std::abs(p1.x - p2.x) < EPSILON) continue;
                ...
                for (size_t c = c_start; c < c_end; ++c) {
                    ...
                    if (y_intersect < value_y[0] - EPSILON) {
                        isInsideX_sub[c_sub] = !isInsideX_sub[c_sub];
                    } 
    ...
    // --- 8. Finalize Outputs (Memory Fix) ---
    if (useYScan) {
        // Y-Scan: cumsum along dimension 2 (X)
        // --- #pragma omp parallel for REMOVED ---
        for (size_t r_sub = 0; r_sub < M_sub; ++r_sub) {
            size_t r = r_sub + r_start_bbox;
            int crossingCount = 0;
            bool parity = isInsideY_sub[r_sub];
            
            for (size_t c_sub = 0; c_sub < N_sub; ++c_sub) {
                size_t c = c_sub + c_start_bbox;
                
                size_t idx_sub = c_sub * M_sub + r_sub;
                size_t idx_full = c * M + r;         
                
                crossingCount += diffCount_sub[idx_sub];
                bool totalInclusion = (crossingCount % 2) != 0;
                totalInclusion = totalInclusion ^ parity;

                if (!onedgeMeshPtr[idx_full]) { 
                    inMeshPtr[idx_full] = totalInclusion;
                }
            }
        }
    } else {
        ...
        for (size_t c_sub = 0; c_sub < N_sub; ++c_sub) {
            size_t c = c_sub + c_start_bbox;
            int crossingCount = 0;
            bool parity = isInsideX_sub[c_sub];
